      ...
      setMessages([
        {
          ...
          role: "model",
          text: "El chat no está disponible ahora mismo porque falta la configuración de Gemini. Podés escribirme por el formulario o por WhatsApp.",
        },
      ]);
      return;
    }
...
Habla como Ezequiel en primera persona, aclarando que sos su versión IA.`,
      ...
      setMessages([
        {
          id: Date.now().toString(),
          role: "model",
          text: "No pude inicializar el chat en este momento. Podés dejar tu mensaje en el formulario de contacto.",
        },
      ]);
    }
  }, [geminiApiKey, isOpen]);
  ...
  const handleInitialGreeting = async () => {
    ...
    try {
      const response = await chatRef.current.sendMessage({
        message: "Hola Ezequiel, me gustaría obtener un presupuesto automático para mi proyecto.",
      });
      ...
      setMessages([
        {
          id: Date.now().toString(),
          role: "model",
          text: "Hola, soy el Avatar de Ezequiel. Estoy acá para ayudarte a obtener un presupuesto automático. ¿Qué proceso te gustaría automatizar hoy?",
        },
      ]);
    } finally {
      setIsLoading(false);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: (Date.now() + 1).toString(), role: "model", text: "Error de conexión. Por favor, intentá de nuevo." },
      ]);
    } finally {
      setIsLoading(false);
    }
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-background/50">
                ...
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                    <div
                      className={`w-8 h-8 flex items-center justify-center shrink-0 rounded-full border overflow-hidden ${msg.role === "user" ? "bg-slate-800 text-slate-300 border-white/10" : "bg-primary/10 text-primary border-primary/20"}`}
                    >
                      {msg.role === "user" ? <User size={16} /> : <Avatar size="lg" />}
                    </div>
                    <div
